...
import { parseStarsInvoicePayload } from '@/lib/starsInvoices';
...
export async function validateStarsPreCheckout(query) {
  const parsedPayload = parseStarsInvoicePayload(query?.invoice_payload);

  if (!parsedPayload || !parsedPayload.validSignature) {
    return { ok: false, error: 'Invoice ma’lumoti noto‘g‘ri.' };
  }

  const queryUserId = Number(query?.from?.id);
  const queryAmount = Number(query?.total_amount);
  const queryCurrency = clean(query?.currency);

  if (parsedPayload.signed) {
    if (
      parsedPayload.userId !== queryUserId ||
      parsedPayload.amount !== queryAmount ||
      queryCurrency !== 'XTR'
    ) {
    ...
    if (
      parsedPayload.expiresAtSeconds > 0 &&
      Math.floor(Date.now() / 1000) > parsedPayload.expiresAtSeconds
    ) {
      return {
        ok: false,
        error: 'Invoice muddati tugagan. Yangisini yarating.',
      };
    }

    // V2 payload user, summa va muddatni HMAC bilan imzolaydi. Shu sabab
    // Telegramning 10 soniyalik pre-checkout oynasida Supabase'ni kutmaymiz.
    return {
      ok: true,
      fast: true,
      deposit: {
        id: parsedPayload.depositId,
        user_id: parsedPayload.userId,
        method: 'stars',
        status: 'pending',
        pay_currency: 'XTR',
        pay_amount: parsedPayload.amount,
      },
    };
  }
  ...
  const { data: deposit, error } = await supabase
    ...
    .eq('invoice_payload', parsedPayload.payload)
    .maybeSingle();
  ...
  if (
    ...
    Number(deposit.user_id) !== queryUserId ||
    queryCurrency !== 'XTR' ||
    Number(deposit.pay_amount) !== queryAmount
  ) {
    return { ok: false, error: 'Invoice ma’lumotlari mos kelmadi.' };
  }
...
export async function markStarsPreCheckout(deposit) {
  if (!deposit?.id || !deposit?.user_id) return null;
  ...
  const { data, error } = await supabase
    ...
    .eq('id', deposit.id)
    .eq('user_id', Number(deposit.user_id))
    .eq('method', 'stars')
    .eq('status', 'pending')
    .select('id,status')
    .maybeSingle();
...
export async function completeStarsPayment(message) {
  ...
  const parsedPayload = parseStarsInvoicePayload(payment?.invoice_payload);

  if (
    !payment ||
    !parsedPayload ||
    !parsedPayload.validSignature ||
    !message?.from?.id
  ) {
  ...
  if (
    parsedPayload.signed &&
    (
      parsedPayload.userId !== Number(message.from.id) ||
      parsedPayload.amount !== Number(payment.total_amount) ||
      clean(payment.currency) !== 'XTR'
    )
  ) {
    throw new Error('Stars to‘lov imzosi invoice ma’lumotlariga mos kelmadi.');
  }

  const depositId = parsedPayload.depositId;
  ...
  const { data, error } = await supabase.rpc('deposit_complete', {
    ...
    p_metadata: {
      ...
      invoicePayload: parsedPayload.payload,
      invoicePayloadVersion: parsedPayload.version,
