...
function catalogRowFromGift(gift, existing = {}, assets = {}) {
  ...
  const floorPrice = toNumber(gift.floorPrice || gift.starCount || gift.stars, 0);
...
export async function syncTelegramGiftCatalog(supabase, options = {}) {
  ...
  return withTelegramClient(async (client) => {
    ...
    for (const gift of normalized) {
      ...
      if (downloadAssets && (!giftAssets.image || !giftAssets.animation)) {
      ...
      } else if (giftAssets.image || giftAssets.animation) {
        reused += 1;
      }
...
export async function createGiftFromCatalog(supabase, options = {}) {
  ...
  const starCount = Math.max(0, toNumber(catalogGift.star_count, floorPrice));
